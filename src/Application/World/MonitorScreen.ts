...
export default class MonitorScreen extends EventEmitter {
    ...
    createIframe() {
        ...
        // Point to the inner portfolio in the dist folder
        iframe.src = './inner-portfolio/';
        /**
    ...
    getVideoTextures(videoId: string) {
        ...
        if (!video) {
            console.log(`Video ${videoId} not found, retrying...`);
            setTimeout(() => {
                this.getVideoTextures(videoId);
            }, 100);
        } else {
            console.log(`Video ${videoId} found, creating texture`);
            this.videoTextures[videoId] = new THREE.VideoTexture(
                video as HTMLVideoElement
            );
            this.videoTextures[videoId].flipY = false;
        }
    }
